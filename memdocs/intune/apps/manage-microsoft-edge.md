...
- На портале Azure перейдите в раздел [Центр администрирования диспетчера конечных точек (Майкрософт)](https://go.microsoft.com/fwlink/?linkid=2109431) -> **Администрирование клиента** -> **Фирменная символика и настройка** -> **Фирменная символика и настройка компании**.
...
|    Key    |    Значение    |
|---------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    `com.microsoft.intune.mam.managedbrowser.defaultHTTPS`     |     Значение **True** задает HTTPS в качестве протокола по умолчанию.     |
...
## <a name="disable-inprivate-and-microsoft-accounts-msa-to-restrict-personal-browsing"></a>Отключение InPrivate и учетных записей Майкрософт (MSA) для ограничения личного просмотра
Некоторые клиенты в отраслях со строгими требованиями, использующие Microsoft Edge, могут ограничить доступ пользователей к просмотру только контекстом AAD. Для отключения учетных записей Майкрософт или просмотра InPrivate можно использовать следующие параметры конфигурации приложения.

|    Key    |    Значение    |
|-------------------------------------------------------------------|-------------------------------------------------------|
|     `com.microsoft.intune.mam.managedbrowser.disabledFeatures`    |    **InPrivate** — отключение браузера InPrivate. <br> **msa** — отключение возможности пользователей добавлять персональные учетные записи MSA в Microsoft Edge.<br> Чтобы отключить учетные записи InPrivate и MSA, используйте `inprivate| msa`    |  


Если вы не развертываете политики защиты приложений, можно также запретить пользователям использовать учетные записи Майкрософт и разрешить просмотр только из рабочей или учебной учетной записи на зарегистрированных устройствах. Дополнительные сведения о ключах для настройки режима "только учетная запись организации" для Microsoft Edge:
- [Только учетная запись организации Android](https://docs.microsoft.com/intune/apps/app-configuration-policies-use-android#allow-only-configured-organization-accounts-in-multi-identity-apps)
- [Только учетная запись организации iOS](https://docs.microsoft.com/intune/apps/app-configuration-policies-use-ios#allow-only-configured-organization-accounts-in-multi-identity-apps)
...
Если ваша организация работает в индустрии со строгими нормативными требованиями или использует VPN для каждого приложения, чтобы предоставлять пользователям доступ к рабочим ресурсам с помощью Microsoft Edge, вы можете выбрать область использования Microsoft Edge только для контекста, защищенного с помощью MAM. Эта функция предоставляется только для устройств, зарегистрированных в MDM.

|    Key    |    Значение    |
|-----------|-------------|
|    `com.microsoft.intune.mam.managedbrowser.disableFeatures`    |    Если установлено значение **inprivate**, просмотр InPrivate отключен. <br> Если установлено значение **msa**, пользователи не могут добавлять свои персональные учетные записи Майкрософт (MSA) в приложения Microsoft Edge. <br> Чтобы отключить несколько компонентов, разделите значения с помощью `|`. Например, `inprivate|msa` будет блокировать InPrivate и личные учетные записи.   |
...
Сведения о том, как просматривать журналы на устройствах с Android, см. в разделе [Отправка журналов ИТ-администратору по электронной почте](https://docs.microsoft.com/user-help/send-logs-to-your-it-admin-by-email-android).
