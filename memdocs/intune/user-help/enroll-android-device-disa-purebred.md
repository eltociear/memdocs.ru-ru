---
title: Регистрация устройства Android с помощью приложения Microsoft Intune и DISA Purebred
description: Узнайте, как зарегистрировать устройство Android и настроить проверку подлинности производных учетных данных с помощью DISA Purebred.
keywords: ''
author: lenewsad
ms.author: lanewsad
manager: dougeby
...
ms.locfileid: "83880622"
---
# <a name="set-up-android-device-with-the-microsoft-intune-app-and-disa-purebred"></a>Настройка устройства Android с помощью приложения Microsoft Intune и DISA Purebred
...
* Настройка смарт-карты путем установки производных учетных данных от поставщика производных учетных данных организации — [DISA Purebred](https://public.cyber.mil/pki-pke/purebred/)
...
* установленное на устройстве приложение Purebred (приложение должно автоматически установиться вскоре после настройки устройства; если оно не установилось, обратитесь к специалисту службы ИТ-поддержки).
...
16. Перейдите к разделу [Настройка смарт-карты](enroll-android-device-disa-purebred.md#set-up-smart-card) этой статьи, чтобы завершить настройку устройства.  
...
> [!NOTE]
> Для выполнения этих действий требуется приложение Purebred, которое будет автоматически установлено на устройстве после регистрации. Если по истечении некоторого времени приложение так и не установилось, обратитесь к специалисту службы ИТ-поддержки.  
...
1. После завершения регистрации приложение Intune сообщит вам о настройке доступа по смарт-карте. Коснитесь уведомления. Если вы не получили уведомление, проверьте свою электронную почту.
   ...
   > [!div class="mx-imgBorder"]
   > ![Снимок экрана: приложение Intune с push-уведомлением на начальном экране устройства](./media/action-required-in-app-android.png)

2. На экране **Настройка смарт-карты** выполните приведенные далее действия.

   1. Перейдите по ссылке на инструкции по настройке, предоставленные вашей организацией, и ознакомьтесь с ними. Если дополнительных инструкций не будет, вам нужно будет выполнить действия, приведенные в этой статье.

   2. Нажмите **НАЧАТЬ**.   

   > [!div class="mx-imgBorder"]
   > ![Снимок экрана: приложение Intune, экран настройки смарт-карты](./media/smart-card-open-disa-purebred-android.png)

3. На экране **Получение сертификатов** нажмите **ЗАПУСТИТЬ PUREBRED**, чтобы открыть приложение Purebred. (Приложение должно быть установлено на устройстве автоматически. Если у вас его нет, обратитесь к специалисту службы поддержки.)  

   > [!div class="mx-imgBorder"]
   > ![Снимок экрана: приложение Intune с запросом на открытие приложения DISA Purebred](./media/open-app-prompt-disa-purbred-android.png)  

4. Для правильной работы приложению Purebred могут потребоваться дополнительные разрешения. При появлении запроса нажмите **Разрешить** или **Разрешать всегда**. Чтобы узнать, для чего нужны эти разрешения, обратитесь к специалисту службы поддержки или агенту Purebred.  

5. Открыв приложение Purebred, вместе с агентом Purebred организации скачайте и установить сертификаты, необходимые для доступа к рабочим или учебным ресурсам.

    > [!IMPORTANT]
    > Во время этого процесса нажимайте **OK** или **Установить** при появлении соответствующего запроса. Не изменяйте имена центров сертификации (ЦС) или сертификатов, которые будут предложены для установки.    

6. После завершения установки вы получите уведомление о том, что сертификаты готовы. Коснитесь уведомления, чтобы вернуться в приложение Intune.

    > [!div class="mx-imgBorder"]
    > ![Снимок экрана: экран разрешения доступа к сертификатам](./media/certificates-ready-prompt-disa-purbred-android.png)

7. На экране **Разрешение доступа к сертификатам** вы предоставите приложению Intune разрешение на доступ к производным учетным данным, полученным от DISA Purebred. Этот шаг гарантирует, что ваша организация может проверять вашу личность при каждом доступе к защищенным рабочим или учебным ресурсам.  

    1. Нажмите кнопку **ДАЛЕЕ**.

       > [!div class="mx-imgBorder"]
       > ![Снимок экрана: уведомление о готовности сертификатов](./media/certificates-access-disa-purbred-android.png)

    2. При появлении запроса **Выберите сертификат** не изменяйте выбор. Правильный сертификат уже выбран, поэтому просто нажмите **Выбрать** или **OK**.  

       > [!div class="mx-imgBorder"]
       > ![Снимок экрана: запрос на выбор сертификата](./media/choose-certificates-prompt-disa-purbred-android.png)

    3. Производные учетные данные состоят из нескольких сертификатов, поэтому запрос **Выберите сертификат** может выводиться несколько раз. Повторяйте предыдущий шаг, пока запросы не перестанут появляться.  

8. После обработки всех сертификатов дождитесь завершения настройки устройства в приложении Intune. Вы узнаете, что установка завершена, когда увидите экран **Все готово!** .  

    > [!div class="mx-imgBorder"]
    > ![Снимок экрана: экран "Все готово"](./media/all-set-android.png)
