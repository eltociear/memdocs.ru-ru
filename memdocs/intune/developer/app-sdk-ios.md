...
* **IntuneMAMSwift.framework**: платформа SWIFT пакета SDK для приложений Intune. Эта платформа содержит все заголовки для API-интерфейсов, которые будут вызываться приложением. Свяжите эту платформу с приложением или расширениями, чтобы включить управление клиентскими приложениями Intune.

* **IntuneMAMSwiftStub.framework**: платформа заглушки SWIFT пакета SDK для приложений Intune. Это обязательная зависимость IntuneMAMSwift.framework, с которой должны быть связаны приложения или расширения.
...
1. **Вариант 1 — платформа (рекомендуется)** . Если вы используете Xcode 10.2 и более поздних версий и ваше приложение или расширение содержит SWIFT-код, свяжите `IntuneMAMSwift.framework` и `IntuneMAMSwiftStub.framework` с целевым объектом: перетащите `IntuneMAMSwift.framework` и `IntuneMAMSwiftStub.framework` в список **Внедренные двоичные файлы** для целевого объекта проекта.

    Или свяжите `IntuneMAM.framework` с целевым объектом: перетащите `IntuneMAM.framework` в список **Внедренные двоичные файлы** для целевого объекта проекта.
   ...
   **Вариант 2 — статическая библиотека**. Этот параметр доступен только для приложений и расширений, которые не содержат SWIFT-кода или были созданы с помощью Xcode до версии 10.2. Выполните связывание с библиотекой `libIntuneMAM.a`. Перетащите библиотеку `libIntuneMAM.a` в список **Linked Frameworks and Libraries** (Связанные платформы и библиотеки) цели проекта.

    ![Пакет SDK для приложений Intune для iOS — связанные платформы и библиотеки](./media/app-sdk-ios/intune-app-sdk-ios-linked-frameworks-and-libraries.png)
...
4. Чтобы создать отдельную группу доступа, в которой будут храниться данные пакета SDK для приложений Intune, включив совместное использование цепочки ключей, выполните приведенные ниже инструкции. Группу доступа к цепочке ключей можно создать при помощи пользовательского интерфейса или файла прав. При использовании пользовательского интерфейса для создания группы доступа к цепочке ключей обязательно выполните следующие действия:
    ...
    б. Добавьте группу общей цепочки ключей `com.microsoft.intune.mam` к существующим группам доступа. Пакет SDK для приложений Intune использует эту группу доступа для хранения данных.
    ...
    г. Если вы изменяете файл прав непосредственно (не с помощью пользовательского интерфейса Xcode, показанного выше) для создания группы доступа к цепочке ключей, добавьте `$(AppIdentifierPrefix)` в начало групп доступа к цепочке ключей (Xcode обрабатывает это значение автоматически). Пример.
...
5. Включите в массив `UIApplication canOpenURL` в файле Info.plist приложения каждый из протоколов, которые приложение передает в `LSApplicationQueriesSchemes`. Не забудьте сохранить изменения перед переходом к следующему шагу.
...
Словарь **IntuneMAMSettings** в файле Info.plist приложения можно использовать для настройки пакета SDK для приложений Intune. Если словарь IntuneMAMSettings не отображается в файле Info.plist, необходимо создать его.
...
--       |  --   |   --       |  --
ADALClientId  | Строка  | Идентификатор клиента Azure AD приложения. | Требуется для всех приложений, использующих MSAL, и любых приложений, использующих ADAL, которые обращаются к ресурсу AAD не в Intune. |
...
В любой момент времени каждый поток имеет действующее удостоверение для задач, связанных с пользовательским интерфейсом, и задач файлов. Это удостоверение используется для определения применяемых политик (если таковые имеются). Если в качестве удостоверения задано значение "no identity" (без удостоверения) либо пользователь не является управляемым, никакие политики не применяются. На рисунках ниже показано, как определяются действующие удостоверения.
  ...
  ![Пакет Intune App SDK для iOS: процесс определения удостоверений](./media/app-sdk-ios/ios-thread-identities.png)
