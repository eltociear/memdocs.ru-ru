...
В Microsoft Intune эти шаблоны встроены в службу Intune и доступны как профили **Административные шаблоны**. В этом профиле вы настраиваете параметры, которые необходимо включить, а затем назначаете этот профиль вашим устройствам.
...
- На локальном контроллере домена Active Directory:
  ...
  2. Создайте групповую политику для отправки этих шаблонов на компьютер администратора Windows 10 Enterprise в том же домене, что и контроллер домена. В этом учебнике вы:
      ...
      В некоторых организациях у администратора домена есть две учетные записи — обычная рабочая учетная запись домена и другая учетная запись администратора домена, используемая только для задач администратора домена, например для групповой политики.
...
2. Перейдите в [центр администрирования Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431) (https://devicemanagement.microsoft.com). Войдите со следующей учетной записью:
...
В следующих шагах вы создадите группы безопасности и добавите в них пользователей. Одного пользователя можно добавить в несколько групп. У пользователя часто бывает несколько устройств, например Surface Pro для работы и мобильное устройство Android для личного пользования. И это нормально, когда пользователь получает доступ к ресурсам организации с нескольких устройств.
...
5. Выберите **Создать группу** и укажите следующие параметры:
    ...
    - **Динамические устройства-члены**: Настройте запрос:
...
6. Создайте группу **Все преподаватели** со следующими параметрами:
    ...
    - **Динамические пользователи-члены**: Настройте запрос:
...
3. Щелкните **Создать**. В раскрывающемся списке **Выбрать категорию** выберите **Все продукты**. Отображаются все параметры. В этих параметрах обратите внимание на следующие свойства:

    - **Путь** политики такой же, как в приложении "Управление групповыми политиками" или GPEdit.
    - Этот параметр применяется к пользователям или устройствам.
...
3. Щелкните правой кнопкой мыши политику **OfficeandEdge** > **Изменить**. Откроется приложение редактора управления групповыми политиками.
    ...
    > [!div class="mx-imgBorder"]
    > ![Щелкните правой кнопкой мыши групповую политику Office и Edge ADMX и выберите "Изменить"](./media/tutorial-walkthrough-administrative-templates/open-group-policy-management.png)
...
5. В центре администрирования управления устройствами перейдите к шаблону **Административный шаблон — устройства Windows 10 для учащихся**.
6. Выберите **Все продукты** из раскрывающегося списка и выполните поиск по запросу **персонализация**:

    > [!div class="mx-imgBorder"]
    > ![Найдите персонализацию в административном шаблоне в Microsoft Intune](./media/tutorial-walkthrough-administrative-templates/search-personalization-administrative-template.png)

    Обратите внимание на доступные параметры.

    Тип параметра — **Устройство**, а путь — **\Панель управления\Персонализация**. Этот путь похож на тот, который вы только что видели в редакторе "Управление групповыми политиками". Если вы откроете этот параметр, вы увидите те же варианты **Не настроено**, **Включено** и **Отключено**, что и в редакторе "Управление групповыми политиками".

#### <a name="compare-a-user-policy"></a>Сравнение политики пользователей

1. В административном шаблоне администратора найдите **Просмотр InPrivate**. Обратите внимание на путь, и этот параметр применяется к пользователям и устройствам.
...
1. В центре администрирования управления устройствами перейдите к шаблону **Административный шаблон — устройства Windows 10 для учащихся**.
2. В раскрывающемся списке выберите **Версия Edge 77 и более поздние**.
3. Найдите **запуск**. Обратите внимание на доступные параметры.
4. В редакторе "Управление групповыми политиками" найдите следующие параметры:
...
1. В шаблоне **Административный шаблон — устройства с Windows 10 для учащихся** найдите **Отключить просмотр InPrivate** и выберите политику устройства:
...
2. В этом окне обратите внимание на описание и значения, которые можно задать. Эти параметры похожи на те, что отображаются в групповой политике.
...
1. В шаблоне выберите **Назначения**. Возможно, потребуется закрыть шаблон, а затем выбрать его из списка **Устройства — профили конфигурации**:
...
2. Щелкните **Выберите группы для включения**. Отобразится список существующих пользователей и групп.
3. Выберите созданную группу **Все устройства с Windows 10 для учащихся** > **Выбрать**.
...
3. Щелкните **Создать**.
4. Выберите в раскрывающемся списке пункт **Офис**.
5. **Включите** следующие параметры. Обязательно нажмите кнопку **OK**, чтобы сохранить изменения.
...
1. В шаблоне выберите **Назначения**.
2. Щелкните **Выберите группы для включения**. Отобразится список существующих пользователей и групп.
3. Выберите созданную группу **Все устройства Windows** > **Выбрать**.
