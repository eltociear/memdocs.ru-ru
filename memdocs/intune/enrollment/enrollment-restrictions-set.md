...
8. Выберите **Далее**, чтобы перейти на страницу **Теги области**.
9. На странице **Теги области** при необходимости добавьте теги области, которые необходимо применить для этого ограничения. Дополнительные сведения о тегах области см. в статье [Use role-based access control (RBAC) and scope tags for distributed IT](../fundamentals/scope-tags.md) (Использование управления доступом на основе ролей (RBAC) и тегов области для распределенной ИТ-разработки). При использовании тегов области с ограничениями регистрации пользователи могут изменять порядок только тех политик, для которых у них есть область. Кроме того, они могут изменять позиции только тех политик, для которых у них есть область. Пользователи видят правильный номер приоритета политики для каждой политики. Пользователи с заданными областями могут указывать относительный приоритет политик, даже если они не видят все остальные политики.
10. Выберите **Далее**, чтобы перейти на страницу **Назначения**.
11. Выберите **Выбор групп для включения**, а затем используйте поле поиска, чтобы найти группы, которые необходимо включить в это ограничение. Это ограничение применяется только к группам, которым оно назначено. Если ограничение не назначено ни одной группе, оно не будет работать. Затем выберите **Выбрать**. 
    ![Снимок экрана выбора настроек платформы](./media/enrollment-restrictions-set/select-groups.png)
12. Выберите **Далее**, чтобы перейти на страницу **Проверка + создание**.
13. Выберите **Создать**, чтобы создать ограничение.
14. Будет создано ограничение с приоритетом выше заданного по умолчанию. Можно [изменить приоритет](#change-enrollment-restriction-priority).
...
6. На странице **Теги области** при необходимости добавьте теги области, которые необходимо применить для этого ограничения. Дополнительные сведения о тегах области см. в статье [Use role-based access control (RBAC) and scope tags for distributed IT](../fundamentals/scope-tags.md) (Использование управления доступом на основе ролей (RBAC) и тегов области для распределенной ИТ-разработки). При использовании тегов области с ограничениями регистрации пользователи могут изменять порядок только тех политик, для которых у них есть область. Кроме того, они могут изменять позиции только тех политик, для которых у них есть область. Пользователи видят правильный номер приоритета политики для каждой политики. Пользователи с заданными областями могут указывать относительный приоритет политик, даже если они не видят все остальные политики.
7. Выберите **Далее**, чтобы перейти на страницу **Назначения**.
8. Выберите **Выбор групп для включения**, а затем используйте поле поиска, чтобы найти группы, которые необходимо включить в это ограничение. Это ограничение применяется только к группам, которым оно назначено. Если ограничение не назначено ни одной группе, оно не будет работать. Затем выберите **Выбрать**. 
    ![Снимок экрана выбора групп](./media/enrollment-restrictions-set/select-groups-device-limit.png)
9. Выберите **Далее**, чтобы перейти на страницу **Проверка + создание**.
10. Выберите **Создать**, чтобы создать ограничение.
11. Будет создано ограничение с приоритетом выше заданного по умолчанию. Можно [изменить приоритет](#change-enrollment-restriction-priority).
