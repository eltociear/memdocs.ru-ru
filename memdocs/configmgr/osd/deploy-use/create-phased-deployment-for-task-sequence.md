...
5. На странице **Этапы** посмотрите два этапа, которые мастер создает для указанной коллекции. Выберите **Далее**.   

    > [!Note]  
    > В этом разделе рассматривается процедура для автоматического создания развертывания в два этапа по умолчанию. Мастер позволяет добавлять, удалять, изменять порядок, редактировать или просматривать этапы для поэтапного развертывания. Сведения о дополнительных действиях см. в разделе о [создании поэтапного развертывания с использованием настраиваемых вручную этапов](#bkmk_manual).  
