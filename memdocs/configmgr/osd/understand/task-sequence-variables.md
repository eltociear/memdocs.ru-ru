...
Используйте в свойствах раздела FAT32 в поле **Переменная**. Обнаружив эту переменную, последовательность задач готовит диск для перехода на UEFI перед перезагрузкой компьютера. Подробные сведения см. в разделе [Этапы последовательности задач для управления преобразованием BIOS в UEFI](../deploy-use/task-sequence-steps-to-manage-bios-to-uefi-conversion.md).
