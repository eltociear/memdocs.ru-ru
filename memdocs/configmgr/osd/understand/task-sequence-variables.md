...
Задает путь, определенный в переменной [SMSTSLocalDataDrive](#SMSTSLocalDataDrive). Если вы определите переменную SMSTSLocalDataDrive перед запуском последовательности задач, например настроив переменную коллекции, то Configuration Manager определит переменную _SMSTSMDataPath в момент запуска последовательности задач.
...
Указывает, где последовательность задач сохраняет временные файлы при выполнении на конечном компьютере.

Обязательно задайте эту переменную перед запуском последовательности задач, например, задав переменную коллекции. Configuration Manager определяет переменную [_SMSTSMDataPath](#SMSTSMDataPath) сразу после запуска последовательности задач.
...
Задает команду, которая выполняется после завершения последовательности задач. Например, вы можете указать скрипт, который включает фильтры записи на встраиваемых устройствах, когда последовательность задач развернет операционную систему на устройстве.
