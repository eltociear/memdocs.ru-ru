...
- [Файлы серверных журналов](#BKMK_ServerLogs)  
  ...
  - [Точка обслуживания хранилища данных](#BKMK_DataWarehouse)
...
|--------------|-----------------|  
|ADALOperationProvider.log|Сведения о запросах маркера проверки подлинности клиента с помощью Библиотеки проверки подлинности Azure Active Directory (ADAL).|
...
|ccmsqlce.log|Записывает действия для выпуска SQL Compact Edition, используемого клиентом. Этот журнал обычно используется только в том случае, если включено ведение журнала отладки или возникла проблема с компонентом. Задача работоспособности клиента (ccmeval) обычно самостоятельно устраняет проблемы с этим компонентом.|
...
|CITaskMgr.log|Записывает задачи для каждого приложения и типа развертывания, такие как скачивание содержимого, установка и удаление.|  
...
### <a name="client-installation"></a><a name="BKMK_ClientInstallLog"></a> Установка клиента
...
|client.msi.log|Записывает задачи настройки, выполняемые пакетом client.msi. Может использоваться для устранения неполадок при установке и удалении клиента.|  
...
|-------------------|-----------------------------------------------------------------|
|Scxcm.log| Файл журнала для службы ядра клиента Configuration Manager для Linux и UNIX (ccmexec.bin). Этот файл журнала содержит сведения об установке и последующей работе ccmexec.bin. По умолчанию этот файл журнала находится здесь: **/var/opt/microsoft/scxcm.log**. Чтобы изменить расположение файла журнала, измените в файле **/opt/microsoft/configmgr/etc/scxcm.conf** поле **PATH** . Нет необходимости перезагружать клиентский компьютер или службу, чтобы изменения вступили в силу. Можно задать четыре уровня ведения журнала. |
| Scxcmprovider.log |Файл журнала для службы CIM клиента Configuration Manager для Linux и UNIX (omiserver.bin). Этот файл журнала содержит сведения о работе nwserver.bin. Этот журнал находится по адресу `/var/opt/microsoft/configmgr/scxcmprovider.log`. Чтобы изменить расположение файла журнала, в файле **/opt/microsoft/omi/etc/scxcmprovider.conf** измените поле **PATH** . Нет необходимости перезагружать клиентский компьютер или службу, чтобы изменения вступили в силу. Можно задать три уровня ведения журнала.|
...
В клиенте Configuration Manager для компьютеров Mac информация записывается в перечисленных ниже файлах журнала на компьютере Mac:  

|Имя журнала|Сведения|Расположение|
|--------------|-------------|-------------|
|CCMClient-&lt;*дата_и_время*>.log|Записывает действия, относящиеся к операциям клиента Mac, среди которых — управление приложениями, инвентаризация и регистрация ошибок.| `/Library/Application Support/Microsoft/CCM/Logs`|  
|CCMAgent-&lt;*дата_и_время*>.log|Записывает сведения, относящиеся к операциям клиента, куда входят операции входа и выхода пользователя из системы и работа компьютера Mac.| `~/Library/Logs`|  
|CCMNotifications-&lt;*дата_и_время*>.log|Содержит сведения о действиях, которые связаны с уведомлениями Configuration Manager на компьютере Mac.| `~/Library/Logs`|  
|CCMPrefPane-&lt;*дата_и_время*>.log|Содержит сведения о действиях, связанных с диалоговым окном настроек Configuration Manager компьютера Mac, куда входят общие сведения о состоянии и ошибки.| `~/Library/Logs`|  
...
|adminservice.log|Журнал действий службы администрирования REST API поставщика SMS.|Компьютер поставщика SMS|
...
|ccm.log|Записывает действия для принудительной установки клиента.|Сервер сайтов|  
...
|CollectionAADGroupSyncWorker.log | Начиная с версии 2002, файл журнала для синхронизации результатов членства в коллекции с каталогом Azure Active Directory. В версии 1910 и более ранних ведение журнала для этой функции было объединено в SMS_AZUREAD_DISCOVERY_AGENT.log. | Сервер сайтов|
...
|distmgr.log|Записывает сведения о создании, сжатии и разностной репликации пакетов, а также об обновлении сведений о пакетах. Он также может включать в себя другие действия из компонента диспетчера распространения. Например, установку точки распространения, попытки подключения и установку компонентов. Дополнительные сведения о других функциях, использующих этот журнал, см. в разделах [Точка подключения службы](#BKMK_WITLog) и [Развертывание ОС](#BKMK_OSDLog).|Сервер сайтов|  
...
|SMS_AZUREAD_DISCOVERY_AGENT.log| Файл журнала для обнаружения пользователей Azure Active Directory (Azure AD) и групп пользователей. В версии 1910 и более ранней он также включал в себя синхронизацию результатов членства в коллекции в Azure AD.| Сервер сайтов|
...
|smstsvc.log|Записывает сведения об установке, использовании и удалении службы Windows. Windows использует эту службу для проверки сетевого подключения и разрешений между серверами с помощью учетной записи сервера, который создает подключение.|Сервер сайта и сервер системы сайта|  
...
### <a name="management-point"></a><a name="BKMK_MPLog"></a> Точка управления
...
|mpMSI.log|Записывает сведения об установке точки управления.|Сервер сайтов|  
|MPSetup.log|В этот файл журнала записывается информация о процессе установки точки управления.|Сервер сайтов|  
|UserService.log|Журнал запросов пользователей из центра программного обеспечения на получение или установку доступных для пользователей приложений с сервера.|Сервер системы сайта|

### <a name="service-connection-point"></a><a name="BKMK_WITLog"></a> Точка подключения службы
...
|Smsprov.log|Записывает действия поставщика SMS. Для действий консоли Configuration Manager используется поставщик SMS.|Компьютер поставщика SMS|  
...
### <a name="asset-intelligence"></a><a name="BKMK_AILog"></a> Аналитика активов
...
Наряду с файлами журнала Configuration Manager просмотрите журналы приложений Windows в средстве просмотра событий на сервере с запущенной службой регистрации сетевых устройств и сервере, где размещена точка регистрации сертификатов. Например, просмотрите сообщения из источника **NetworkDeviceEnrollmentService** .

Можно также использовать следующие файлы журнала:  

- Файлы журнала IIS, относящиеся к службе регистрации сертификатов для сетевых устройств: **%SYSTEMDRIVE%\inetpub\logs\LogFiles\W3SVC1**  

- Файлы журнала служб IIS для точки регистрации сертификатов: **%SYSTEMDRIVE%\inetpub\logs\LogFiles\W3SVC1**  

- Файл журнала политики регистрации сетевых устройств: **mscep.log**  

    > [!NOTE]  
    > Этот файл находится в папке профиля учетной записи NDES, например, C:\Users\SCEPSvc. Для получения дополнительной информации о том, как активировать ведение журналов NDES, см. раздел [Включение ведения журналов](https://social.technet.microsoft.com/wiki/contents/articles/9063.active-directory-certificate-services-ad-cs-network-device-enrollment-service-ndes.aspx#Enable_Logging) в вики NDES.  

### <a name="client-notification"></a><a name="BKMK_BGB"></a> Уведомление клиентов
...
|Имя журнала|Описание:|Компьютер с файлом журнала|
|--------------|-----------------|----------------------------|  
|CloudMgr.log|Записывает сведения о развертывании службы шлюза управления облаком, текущем состоянии службы, а также данные об использовании службы. Чтобы настроить уровень ведения журнала, измените для **Уровень ведения журнала** значение в следующем разделе реестра: `HKLM\SOFTWARE\ Microsoft\SMS\COMPONENTS\ SMS_CLOUD_ SERVICES_MANAGER`|Папка *installdir* на сервере первичного сайта или сайта центра администрирования.|
|CMGSetup.log <sup>[Прим. 1](#bkmk_note1)</sup>|Записывает сведения о втором этапе развертывания шлюза управления облаком (локальное развертывание в Azure). Уровень ведения журнала можно настроить с помощью параметра **Уровень трассировки** (варианты значений: **Сведения** (по умолчанию), **Подробный**, **Ошибки**) на вкладке **Azure portal\Cloud services configuration** (Конфигурация портала Azure и облачных служб).|Папка **%approot%\logs** на сервере Azure или папка SMS/Logs на сервере системы сайта|
|CMGService. log <sup>[Прим. 1](#bkmk_note1)</sup>|Фиксирует сведения об основных компонентах служб в шлюзе управления облаком в Azure. Уровень ведения журнала можно настроить с помощью параметра **Уровень трассировки** (варианты значений: **Сведения** (по умолчанию), **Подробный**, **Ошибки**) на вкладке **Azure portal\Cloud services configuration** (Конфигурация портала Azure и облачных служб).|Папка **%approot%\logs** на сервере Azure или папка SMS/Logs на сервере системы сайта|
...
|CMGContentService.log <sup>[Прим. 1](#bkmk_note1)</sup>|<!--SCCMDocs-pr issue #2822-->Если вы включаете предоставление содержимого шлюзом CMG из службы хранилища Azure, в этом журнале регистрируются сведения о службе.|Папка **%approot%\logs** на сервере Azure или папка SMS/Logs на сервере системы сайта|
...
|Smsprov.log|Записывает действия поставщика SMS. Для действий консоли Configuration Manager используется поставщик SMS.|Сервер сайта или сервер системы сайта|  
...
|smsdpusage.log|В журнал записываются сведения о выполняющемся файле smsdpusage.exe, который предназначен для сбора данных для сводного отчета об использовании точек распространения.|Роль системы сайта|  

### <a name="desktop-analytics"></a>Аналитика компьютеров

Используйте следующие файлы журналов для устранения неполадок с порталом Аналитики компьютеров, интегрированным с Configuration Manager.

Файлы журнала в точке подключения службы находятся в следующем каталоге: `%ProgramFiles%\Configuration Manager\Logs\M365A`.
Файлы журнала в клиенте Configuration Manager находятся в следующем каталоге: `%WinDir%\CCM\logs`.

| Журнал | Описание: |Компьютер с файлом журнала|
|---------|---------|---------|
| M365ADeploymentPlanWorker.log | Сведения о синхронизации плана развертывания из облачной службы Аналитики компьютеров в локальную среду Configuration Manager |Точка подключения службы|
| M365ADeviceHealthWorker.log | Сведения о загрузке сведения о работоспособности устройства из Configuration Manager в облако Майкрософт |Точка подключения службы|
| M365AHandler.log | Сведения о политике параметров Аналитики компьютеров |Клиент|
| M365AUploadWorker.log | Сведения о загрузке коллекции и устройства из Configuration Manager в облако Майкрософт |Точка подключения службы|
| SmsAdminUI.log | Сведения о работе консоли Configuration Manager, например для настройки облачных служб Azure  |Точка подключения службы|

### <a name="discovery"></a><a name="BKMK_DiscoveryLog"></a> Обнаружение
...
### <a name="migration"></a><a name="BKMK_MigrationLog"></a> Миграция
...
### <a name="mobile-devices"></a><a name="BKMK_MDMLog"></a> Мобильные устройства

В следующей таблице перечислены файлы журналов, содержащие сведения об управлении мобильными устройствами.  

#### <a name="enrollment"></a><a name="BKMK_EnrollmentLog"></a> Регистрация
...
#### <a name="exchange-server-connector"></a><a name="BKMK_ExchSrvLog"></a> Коннектор Exchange Server
...
#### <a name="mobile-device-legacy"></a><a name="BKMK_MDLegLog"></a> Устаревшие мобильные устройства
...
### <a name="os-deployment"></a><a name="BKMK_OSDLog"></a> Развертывание ОС

В следующей таблице перечислены файлы журналов, содержащие сведения о развертывании ОС.  
...
|MP_ClientIDManager.log|Записывает ответы точки управления на запросы идентификатора клиента, которые инициируют последовательности задач из PXE или с загрузочного носителя.|Сервер системы сайта|  
...
|Setupact.log|Записывает сведения о журнале Windows Sysprep и журнале установки. Дополнительные сведения см. в статье о [файлах журнала](https://docs.microsoft.com/windows/deployment/upgrade/log-files).|Клиент|  
...
### <a name="software-metering"></a><a name="BKMK_MeteringLog"></a> Контроль использования программных продуктов

В следующей таблице перечислены файлы журналов, содержащие сведения о контроле использования программного обеспечения.  
...
### <a name="software-updates"></a><a name="BKMK_SU_NAPLog"></a> Обновления программного обеспечения

В следующей таблице перечислены файлы журналов, содержащие сведения об обновлениях программного обеспечения.  

|Имя журнала|Описание:|Компьютер с файлом журнала|  
|--------------|-----------------|----------------------------|  
|AlternateHandler.log|Регистрирует данные о том, когда клиент вызывает Office через визуальный COM-интерфейс для загрузки и установки обновлений Приложений Microsoft 365 для предприятия. Это напоминает использование WuaHandler, когда он обращается к Windows Update Agent API для загрузки и установки обновлений Windows.<!-- SCCMDocs#888 -->|Клиент|
|ccmperf.log|Записывает действия, связанные с обслуживанием и захватом данных счетчиков производительности клиентов.|Клиент|
|DeltaDownload.log|Записывает сведения о скачивании экспресс-обновлений и обновлениях, скачанных с помощью оптимизации доставки.|Клиент|  
|PatchDownloader.log|Записывает сведения о процессе загрузки обновлений программного обеспечения из источника обновлений в конечную папку загрузки на сервере сайта.|При скачивании обновлений вручную этот файл журнала находится в каталоге %temp% пользователя, запустившего консоль, на компьютере, где вы используете консоль. В случае правил автоматического развертывания файл журнала находится на сервере сайта в каталоге %windir%\CCM\Logs, если клиент Configuration Manager установлен на сервере сайта.|  
...
### <a name="wake-on-lan"></a><a name="BKMK_WOLLog"></a> Пробуждение по локальной сети
