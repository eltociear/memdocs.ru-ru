...
|Имя журнала|Описание:|Компьютер с файлом журнала|
|--------------|-----------------|----------------------------|  
|CloudMgr.log|Записывает сведения о развертывании службы шлюза управления облаком, текущем состоянии службы, а также данные об использовании службы.<br>Вы можете настроить уровень ведения журнала, изменив значение **Уровень ведения журнала** в разделе реестра HKLM\SOFTWARE\ Microsoft\SMS\COMPONENTS\ SMS_CLOUD_ SERVICES_MANAGER|Папка *installdir* на сервере первичного сайта или сайта центра администрирования.|
|CMGSetup.log<sup>[Примечание 1](#bkmk_note1)</sup>|Записывает сведения о втором этапе развертывания шлюза управления облаком (локальное развертывание в Azure).<br>Уровень ведения журнала можно настроить с помощью параметра **Уровень трассировки** (варианты значений: **Сведения** (по умолчанию), **Подробный**, **Ошибки**) на вкладке **Azure portal\Cloud services configuration** (Конфигурация портала Azure и облачных служб).|Папка **%approot%\logs** на сервере Azure или папка SMS/Logs на сервере системы сайта|
|CMGHttpHandler.log<sup>[Примечание 1](#bkmk_note1)</sup>|Фиксирует сведения о привязке обработчика HTTP-данных в шлюзе управления облаком к службам IIS в Azure<br>Уровень ведения журнала можно настроить с помощью параметра **Уровень трассировки** (варианты значений: **Сведения** (по умолчанию), **Подробный**, **Ошибки**) на вкладке **Azure portal\Cloud services configuration** (Конфигурация портала Azure и облачных служб).<br>Начиная с версии 1806, этот журнал отсутствует. Функциональные возможности этого компонента перенесены в компонент службы шлюза управления облачными клиентами. См. файл CMGService.log.<!--SCCMDocs-pr issue #2822-->|Папка **%approot%\logs** на сервере Azure или папка SMS/Logs на сервере системы сайта|
|CMGService.log<sup>[Примечание 1](#bkmk_note1)</sup>|Фиксирует сведения об основных компонентах служб в шлюзе управления облаком в Azure<br>Уровень ведения журнала можно настроить с помощью параметра **Уровень трассировки** (варианты значений: **Сведения** (по умолчанию), **Подробный**, **Ошибки**) на вкладке **Azure portal\Cloud services configuration** (Конфигурация портала Azure и облачных служб).|Папка **%approot%\logs** на сервере Azure или папка SMS/Logs на сервере системы сайта|
|SMS_Cloud_<br>ProxyConnector.log|Содержит сведения об установлении соединений между службой шлюза управления облаком и точкой соединения шлюза управления облаком.|Сервер системы сайта|
|CMGContentService.log<sup>[Примечание 1](#bkmk_note1)</sup>|<!--SCCMDocs-pr issue #2822-->Если вы включаете предоставление содержимого шлюзом CMG из службы хранилища Azure, в этом журнале регистрируются сведения о службе.|Папка **%approot%\logs** на сервере Azure или папка SMS/Logs на сервере системы сайта|
...
|MP_ClientIDManager.log|Записывает ответы точки управления на запросы идентификатора клиента, которые последовательности задач инициируют из PXE или с загрузочного носителя.|Сервер системы сайта|  
